...
public:
	...
	virtual void generate_all() {
		...
		out.str("");
		out << "cmake_minimum_required(VERSION 2.0)" << endl << endl;
	...
	virtual void generate(Type* type) {
		...
		update(header_dir, type->name + (p_iface ? "Support" : "") + ".hxx", out.str());
		
		// generate source files
		out.str("");
		generate_source(type);
		string source_dir = "src/" + subs(type->package->name, ".", "/") + "/";
		string source_name = type->name;
		if(p_iface) {
	...
	void generate_source(Type* p_type) {
		...
		string header = "\n";
		if(p_iface && p_iface->generic.size()) {
			scope += "<";
			header += "template<";
			int i = 0;
